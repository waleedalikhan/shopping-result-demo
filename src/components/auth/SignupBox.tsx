...
const SignupBox: React.FC<Props> = ({ onSigninClick, onSignupSuccess }) => {
  const http = Axios
  ...
  const updateSignupInfo = (
    e: any,
    valueToChange: 'name' | 'email' | 'password',
  ) => {
    let updatedSignupInfo: { name: string; email: string; password: string } = {
      ...signupInfo,
    }
    updatedSignupInfo[valueToChange] = e.target.value
    setSignupInfo((signupInfo = updatedSignupInfo))
  }
  ...
  const handleSignup = async () => {
    ...
    try {
      let res = await http.post(`${HOST_NAME}/api/register`, signupInfo)
      if (res) {
        toast(
          'Account has been created, Please check you email account and verify it!',
        )
      }
                        ...
                        <div className="mt-1">
                          <div className="group relative flex h-12">
                            ...
                            <input
                              ...
                              onChange={(e: any) => updateSignupInfo(e, 'name')}
                            />
                          </div>
