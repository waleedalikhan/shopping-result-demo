...
import LinkWithoutIcon from '@/components/global/LinkWithoutIcon'
...
const LoginBox: React.FC<Props> = ({
  ...
  let [loginForm, setLoginForm] = useState<{ email: string; password: string }>(
    {
      email: '',
      password: '',
    },
  ...
  const updateLoginForm = (e: any, valueToChange: 'email' | 'password') => {
    let updatedLoginForm: { email: string; password: string } = { ...loginForm }
    updatedLoginForm[valueToChange] = e.target.value
    setLoginForm((loginForm = updatedLoginForm))
  }
  ...
  const handleSignIn = async (e: any) => {
    ...
    try {
      let response = await http.post(`${HOST_NAME}/api/login`, loginForm)
                  ...
                  <div className="bg-white  sm:rounded-lg sm:px-2">
                    <form className="space-y-6" onSubmit={handleSignIn}>
                        ...
                        <div className="mt-1">
                          <div className="group relative flex h-12">
                            ...
                            <input
                              ...
                              onChange={(e: any) => updateLoginForm(e, 'email')}
                            />
                          </div>
                          <div className="group relative flex h-12 mt-2">
                            <SecurityKeyIcon />
                            <input
                              className=" flex-auto appearance-none bg-transparent pl-10 text-zinc-900  placeholder:text-zinc-500 focus:w-full  dark:text-white sm:text-sm &::-webkit-search-cancel-button]:hidden &::-webkit-search-decoration]:hidden &::-webkit-search-results-button]:hidden &::-webkit-search-results-decoration]:hidden pr-4 rounded-md border border-gray-300 "
                              autoComplete="off"
                      ...
                      <div className="flex items-center lg:flex-nowrap flex-wrap justify-between">
                        ...
                        <div className="text-sm space-x-2 flex items-center justify-between md:w-auto w-full">
                          <SecondaryButton
                            type="button"
